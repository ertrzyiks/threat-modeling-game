...
function Card({ code, group, value, description, className }: Props) {
  ...
  return (
    <div className={['transition-all duration-300 ease-out w-64 h-96 border-solid border-2 border-current relative', className, `card-${code}`, highValue ? 'bg-current' : 'bg-white'].join(' ')}>
      ...
      <div className={['mt-2 px-4 text-md', highValue ? 'text-white' : 'text-black'].join(' ')}>
        {description}
      </div>
